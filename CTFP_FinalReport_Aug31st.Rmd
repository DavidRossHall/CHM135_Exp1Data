...
- *Tidying* their data and setting up their worksheets. This step consist of formatting cells to properly display values and handling missing data.^[Specifically, missing data is stored as -999 in NAPS data sets but this value is literally interpreted by Excel, confounding data visualization/analysis.] 
...
- discuss difference between winter and summer data sets (figure 2b and 2c)
- excel functionality (maybe list)


```{r,fig.margin=TRUE, echo = FALSE, message = FALSE, warning=FALSE, fig.height = 6, fig.cap = "Example plots students are expected to create. (A) time-series of pollutants across 7 winter days. (B) Correlation plot of O3 and NO2 concentrations with linear regression in the winter and (C) summer."}
...
p <- as_ggplot(gt) +                                # transform to a ggplot
  draw_plot_label(label = c("A", "B", "C"), size = 16,
                  x = c(0, 0, 0), y = c(1, 0.68, 0.35)) # Add labels
p
...
- Review student submitted work showed most of them understood what was going on, although a handful of errors (i.e. applying linear regression to time series data. )
